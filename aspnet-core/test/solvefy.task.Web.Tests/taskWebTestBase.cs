...
{
    public abstract class taskWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static taskWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(taskWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<taskDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<taskDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<taskDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<taskDbContext, Task<T>> func)
        {
            T result;
